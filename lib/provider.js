...
export default class AcidProvider {
    ...
    constructor () {
        ...
        this.lambdas = ['putchar', 'getchar', 'abort', 'eq', 'neq', 'le', 'lt', 'gt', 'ge', 'and', 'or', 'xor', 'not',
                        'exp', 'ln', 'sqrt', 'sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'sinh', 'cosh', 'tanh'];
                ...
                walk (dir, (err, files) => {
                    ...
                    for (file of files) {
                        if (file.endsWith('.acid') && file != editor.getPath()) {
                            let useName = file.replace(dir, '');
                            let fileName = useName.replace('\\', '/').replace('/', '');
                            while (useName.includes ('/') || useName.includes ('\\')) useName = useName.replace('\\', '/').replace('/', '::');
                            useName = useName.replace('.acid', '').replace(/^::/, '');
                            suggestions.push({
                                text: useName,
                                type: 'import',
